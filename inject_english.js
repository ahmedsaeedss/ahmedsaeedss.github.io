...
async function appendEnglishSubject() {
    ...
    // 2. Find the injection point at the end of the array inside the file
    // The file ends with:
    //         ]
    //     }
    // ];
    // We want to insert our new object right before the last closing bracket.

    // Using a reliable regex to replace the final `];` with our new object.
    const insertionPoint = /\s*\];\s*$/;

    // Stringify the new category
    const englishString = JSON.stringify(englishSubject, null, 4);

    // Replace and prepend a comma to separate from the previous object
    const finalContent = fileContent.replace(insertionPoint, ',\n' + englishString + '\n];');

    console.log("Writing changes to data.js...");
    fs.writeFileSync('data.js', finalContent, 'utf8');
