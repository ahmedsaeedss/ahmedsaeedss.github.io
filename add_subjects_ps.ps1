...
if ($lastIdx -ne -1) {
    # If the line before has a trailing comma, remove the comma from our string to avoid syntax error
    # Or just ensure it is properly joined. We put a comma at the start, so we just remove the trailing comma from the previous object if one exists.
    ...
    Write-Host "Injected English and Everyday Science successfuly."
}
else {
