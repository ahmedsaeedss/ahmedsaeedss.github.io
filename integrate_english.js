...
function integrateEnglishCategories() {
    ...
    try {
        ...
        // 1. Remove the "English" subject I appended at the very end
        let cleanArray = arr.filter(s => s.name !== "English");

        // 2. Find the original "English Language" subject
        let engLang = cleanArray.find(s => s.name === "English Language");
        if (!engLang) {
            console.log("Error: 'English Language' subject not found!");
            return;
        }

        // Rename it to just "English" as requested by user's terminology
        engLang.name = "English";

        // Let's define the 50 distinct category questions manually
        const prepositions = [
        ...
        // Let's integrate into engLang.subcategories

        // 1. Prepositions & Phrasal Verbs
        engLang.subcategories.push({
        ...
        // 4. "Sentence Structure" already exists in the original "English Language"!
        let existingSentenceStruct = engLang.subcategories.find(c => c.category === "Sentence Structure");
        if (existingSentenceStruct) {
            existingSentenceStruct.questions = existingSentenceStruct.questions.concat(sentenceStructs);
        } else {
            engLang.subcategories.push({
                category: "Sentence Structure",
                icon: "fa-align-left",
                questions: sentenceStructs
            });
        }
        ...
        const newStr = "const mainQuizData = \n" + JSON.stringify(cleanArray, null, 4) + ";\n";
        fs.writeFileSync(dataPath, newStr, 'utf8');

        console.log("Integration successful! Modified 'English Language' subject.");
