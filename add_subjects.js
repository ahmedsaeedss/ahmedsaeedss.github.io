...
if (lastBracketIndex !== -1) {
    ...
    // add a comma if the last character is a closing brace
    if (before.endsWith('}')) {
        before += ',';
    }

    // remove the "const newSubjects = [" and "];" from the new data
    let jsonStr = JSON.stringify(newSubjects, null, 4);
    jsonStr = jsonStr.substring(jsonStr.indexOf('[') + 1, jsonStr.lastIndexOf(']'));

    const finalContent = before + "\n" + jsonStr + "\n];\n";
    fs.writeFileSync(mainFile, finalContent, 'utf8');
    console.log('Successfully appended English and Everyday Science to data.js');
} else {
    console.error('Could not find the end of the array in data.js');
}
