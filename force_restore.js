...
function forceRestoreEnglish() {
    console.log("Reading data.js for forced restore...");
    ...
    try {
        ...
        let ObjectArray = JSON.parse(jsonStr);

        // Remove existing English subject completely
        let cleanArray = ObjectArray.filter(s => s.name !== "English");

        // Let's define the 50 questions manually from the original successful injection:
        const prepositions = [
        ...
        // Combine them all
        const all50Questions = [...prepositions, ...oneWord, ...partsOfSpeech, ...sentenceStructs, ...readingComp];

        const newEnglishSubject = {
            name: "English",
            icon: "fa-language",
            subcategories: [
                {
                    ...
                    icon: "fa-book-open",
                    questions: all50Questions
                }
            ]
        };

        cleanArray.push(newEnglishSubject);

        const newStr = "const mainQuizData = \n" + JSON.stringify(cleanArray, null, 4) + ";\n";
        fs.writeFileSync(dataPath, newStr, 'utf8');

        console.log("Forced restoration successful! Total English questions: " + all50Questions.length);
