...
function inject() {
    ...
    if (startIdx === -1) {
        console.error('Could not find starting variable in data.js');
        return;
    }
    ...
    for (const mainCat of data) {
        ...
        for (const subCat of mainCat.subcategories) {
            const catName = subCat.category;
            if (newQuestions[catName]) {
                if (!subCat.questions) subCat.questions = [];
                subCat.questions.push(...newQuestions[catName]);
                itemsAdded += newQuestions[catName].length;
            }
        }
    }
    ...
    console.log(`Successfully injected ${itemsAdded} MCQs!`);
}
